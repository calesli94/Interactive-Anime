...
function isBlockedPage(url=''){
  return /^(edge|chrome|about|extensions):\/\//.test(url) || url.startsWith('chrome-extension://') || url.startsWith('edge-extension://');
}
...
async function sendToContent(message){
  ...
  if(isBlockedPage(tab.url||'')){
    return {ok:false,error:'当前页面是浏览器内部页面，插件无法读取，请打开 BOSS 页面或普通网页测试'};
  }
  ...
  try{
    ...
    return {ok:false,error:`content.js 注入失败：${e?.message||e}`};
  }
