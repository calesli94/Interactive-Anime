...
function contextStatus(){
  ...
  if(!q.name) return 'fresh';
  return q.name===o.name ? 'matched' : 'mismatch';
}
...
function findQueueIndexForCandidate(candidate){
  ...
  if(idx<0 && candidate?.name){
    const ageVal=String(candidate?.age??'');
    idx=state.recommendQueue.findIndex((item)=>{
      const c=queueCandidateFromItem(item);
      const sameName=(c.name||'')===candidate.name;
      if(!sameName) return false;
      if(ageVal==='') return true;
      return String(c.age??'')===ageVal;
    });
  }
  return idx;
}
