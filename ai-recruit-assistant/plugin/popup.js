...
function feedback(msg){ console.warn(msg); $('action-feedback').textContent=msg; }
function textOrDash(v){ return v ? String(v) : '-'; }

async function sendToActiveTab(type,payload={}){
  const [tab]=await chrome.tabs.query({active:true,currentWindow:true});
  if(!tab?.id) return null;
  return new Promise((resolve)=>chrome.tabs.sendMessage(tab.id,{type,...payload},(res)=>{
...
async function refreshContext(){
  const ctx=await sendToActiveTab('EXTRACT_PAGE_CONTEXT');
  if(!ctx?.ok){ feedback('无法读取当前页面上下文，请刷新页面或确认插件已注入'); return; }
...
async function refreshJob(){
  const res=await sendToActiveTab('EXTRACT_JOB');
...
async function generateMessages(){
  ...
  try{
    const data=await api('/api/message/generate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({candidate:state.candidate,job_config:jobConfigForApi(),priority_result:state.priorityResult,context_id:state.contextId})});
...
async function fillMessage(message,strategy){
  const res=await sendToActiveTab('FILL_GREETING',{text:message});
  if(res?.ok){
...
async function analyzeChat(){
  const res=await sendToActiveTab('EXTRACT_CHAT');
